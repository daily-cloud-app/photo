...
これらは**リファレンス用のサンプル**であり、本番運用向けのコードではありません。動作はしますがシンプルな構成です。実利用の前に、セキュリティ・コンプライアンス・スケーリング・コストなどをご自身の要件に合わせて確認・強化・調整してください。**現状のまま（"as is"）無保証**で提供され、利用は自己責任です。クラウド利用で課金が発生する場合があるため、請求の監視と不要リソースの削除をお願いします。
