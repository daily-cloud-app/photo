...
// the backend: "users" (username -> email / Entra id mapping) and "photos".
...
param databaseName string = 'dailycloudphoto'
...
// Photos are partitioned by /userId, which holds the stable Entra object id.
resource photosContainer 'Microsoft.DocumentDB/databaseAccounts/sqlDatabases/containers@2024-05-15' = {
  parent: database
  name: photosContainerName
  properties: {
    resource: {
      id: photosContainerName
      partitionKey: {
        paths: [ '/userId' ]
        kind: 'Hash'
      }
